...
CONFIG_LOADER = config.ConfigLoader(SYSTEM_SETTINGS)
...
def add_system_settings(request):
    ...
    settings = [SystemSetting(setting_id=s[0], setting_name=s[1], description=s[2], value=s[3], default=s[4]) for s in settings]
